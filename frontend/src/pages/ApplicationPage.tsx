...
export function ApplicationPage() {
  ...
  const query = useQuery({ queryKey: ['application', user.data?.id, id], queryFn: () => api.application(id), refetchInterval: 10000 })
  if (query.isPending) return <LoadingState />
  if (query.isError && axios.isAxiosError(query.error) && query.error.response?.status === 404) return <div className="empty-state"><h1>Application unavailable</h1><p>This application was not found or is not accessible to this account.</p><Link to="/applications">My applications</Link></div>
  ...
  return <div className="receipt">
    ...
    <div className="receipt-links"><Link className="button secondary" to="/"><ArrowLeft size={16} />Keep exploring</Link>{user.data?.role === 'STAFF' ? <Link to="/staff">Open staff review →</Link> : <Link to="/applications">My applications</Link>}</div>
  </div>
}
