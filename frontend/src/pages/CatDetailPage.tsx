...
export function CatDetailPage() {
  ...
  const available = ['NORMAL', 'ADOPTABLE'].includes(cat.status) && !cat.hasActiveAlert
  ...
  return <>
    ...
    <div className="detail-layout">
      ...
      <div className="detail-copy"><p className="eyebrow">YOUR NEXT CHAPTER STARTS HERE</p><h1>Meet {cat.name}.</h1><StatusBadge status={cat.status} />
        ...
        {cat.hasActiveAlert && <p className="notice">Our team is reviewing a care alert. Adoption is paused until the review is complete.</p>}
        ...
        </form> : <div className="notice"><strong>{cat.status === 'ADOPTED' ? 'A home has been found.' : 'Taking a little time for care.'}</strong><p>{cat.status === 'ADOPTED' ? 'Explore the other cats waiting for their next chapter.' : 'Applications will reopen when the shelter team confirms this cat is ready.'}</p><Link to="/">Meet other cats →</Link></div>}
